...
import 'package:daily_news_api/client.dart';
import 'package:http/http.dart' as http;

/// {@template daily_news_api_malformed_response}
/// An exception thrown when there is a problem decoded the response body.
/// {@endtemplate}
class DailyNewsApiMalformedResponse implements Exception {
  /// {@macro daily_news_api_malformed_response}
  const DailyNewsApiMalformedResponse({required this.error});

  /// The associated error.
  final Object error;
}

/// {@template daily_news_api_request_failure}
/// An exception thrown when an http request failure occurs.
/// {@endtemplate}
class DailyNewsApiRequestFailure implements Exception {
  /// {@macro daily_news_api_request_failure}
  const DailyNewsApiRequestFailure({
...
/// {@template daily_news_api_client}
/// A Dart API client for the Daily News API.
/// {@endtemplate}
class DailyNewsApiClient {
  /// Create an instance of [DailyNewsApiClient] that integrates
  /// with the remote API.
  ///
  /// {@macro daily_news_api_client}
  DailyNewsApiClient({
  ...
  Future<ArticleResponse> getArticle({
    ...
    if (response.statusCode != HttpStatus.ok) {
      throw DailyNewsApiRequestFailure(
        body: body,
  ...
  Map<String, dynamic> json() {
    try {
      ...
      Error.throwWithStackTrace(
        DailyNewsApiMalformedResponse(error: error),
        stackTrace,
      );
    }
  }
